    ...
    <div>  
      <img src="<?php if (!empty($user["image"])) {
        echo strpos($user["image"], "res.cloudinary.com") !== false
          ...
          : BASEURL . "/" . ltrim(htmlspecialchars($user["image"]), "/"); // Tambah BASEURL untuk path lokal
      } else {
        echo BASEURL . "/assets/img/default.jpg"; // Fallback jika kosong
      } ?>"
alt="Profile Image"
width="64"
height="64"
...
style="object-fit: cover;">
 ...
 <h4 class="mb-3">My Addresses</h4>  
  ...
  $address["country"]
      ...
      <small>Phone: <?= htmlspecialchars(
        $address["phone_number"]
      ) ?></small><br>  <div class="mt-3">  
    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editAddressModal<?= $address[
      "id"
    ] ?>">Edit</button>  
    <form action="<?= BASEURL ?>/user/deleteAddress/<?= $address[
  "id"
] ?>" method="POST" class="d-inline" onsubmit="return confirm('Hapus alamat ini?')">  
...
<div class="modal fade userModal" id="editAddressModal<?= $address[
  ...
  <div class="modal-dialog modal-lg">
    <form class="modal-content" method="POST" action="<?= BASEURL ?>/user/updateAddress/<?= $address[
  "id"
] ?>">
      ...
      <div class="modal-body row g-3">
        ...
        <div class="col-12 form-check">
          <input type="checkbox" class="form-check-input" name="is_default" value="1" <?= $address[
            "is_default"
          ]
            ? "checked"
            : "" ?>>
