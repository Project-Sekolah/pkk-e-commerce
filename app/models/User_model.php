...
class User_model {
    ...
    public function updateProfile($id, $username, $full_name, $email, $image = null) {
    ...
    public function updateUserAddress($id, $userId, $label, $address_line_1, $address_line_2, $city, $postal_code, $country, $phone_number, $is_default) {
        if ($is_default) {
    ...
    public function addUserAddress($userId, $label, $address_line_1, $address_line_2, $city, $postal_code, $country, $phone_number, $is_default) {
        if ($is_default) {
            $this->unsetOtherDefaultAddresses($userId);
        }
    ...
    // Soft delete an address (set deleted_at timestamp)
    public function deleteUserAddress($addressId, $userId) {
        $sql = "UPDATE user_addresses SET deleted_at = CURRENT_TIMESTAMP WHERE id = :id AND user_id = :user_id AND deleted_at IS NULL";
        $this->db->query($sql);
    ...
    // Cleanup function to remove soft-deleted addresses older than 7 days
    public function cleanupSoftDeletedAddresses() {
        $sql = "DELETE FROM user_addresses WHERE deleted_at IS NOT NULL AND deleted_at < NOW() - INTERVAL 7 DAY";
        $this->db->query($sql);
        return $this->db->execute();
    }
}
